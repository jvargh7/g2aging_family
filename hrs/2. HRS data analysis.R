...
?cor.test
cor.test(coupleHRS$hh_children, coupleHRS$hh_size)
...
require(srvyr)
colnames(coupleHRS)
source("svysummary.R")#need to download the latest function from github
...
colnames(coupleHRS)
coupleHRS[, c(17:24, 41:44, 53, 54, 57:59, 62, 63, 66, 67)] <- lapply(coupleHRS[, c(17:24, 41:44, 53, 54, 57:59, 62, 63, 66, 67)], factor)

before_imputation <- coupleHRS %>% 
  dplyr::select(hhid, HHIDPN, S13HHIDPN, strata_weight, PSU_weight, hh_weight, r_indweight, s_indweight,
                one_of(continuous_vars), one_of(proportion_vars), one_of(grouped_vars)) %>% 
  mutate_at(vars(strata_weight, w_moderate_pa,w_vigorous_pa,
                 h_moderate_pa,h_vigorous_pa,
                 h_heavydrinker,w_heavydrinker,
                 residence, hh_lengthmar),~as.numeric(.)) %>% #what is this for?
...
pred[c("hhid", "HHIDPN", "S13HHIDPN", "strata_weight", "PSU_weight", "hh_weight", "r_indweight", "s_indweight"),] <- 0
pred[,c("hhid", "HHIDPN", "S13HHIDPN", "strata_weight", "PSU_weight", "hh_weight", "r_indweight", "s_indweight")] <- 0
...
saveRDS(mi_dfs, "G2A HRS Couples mi_dfs.RDS")
