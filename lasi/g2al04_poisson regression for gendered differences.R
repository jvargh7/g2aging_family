...
g_covariates = "+ bmi + ge65 + education_2 + education_3 + laborforce + smoke + heavydrinker + moderate_pa + vigorous_pa"
hhg_covariates = "+ hh_low + hh_medium + hh_high + hh_highest + hh_size + residence + hh_lengthmar + in_religion + in_caste"


## IMPORTANT - Check if coupleid is nested within PSU
## Before proceeding, check variance partition between 'psu' and 'coupleid'

lasi_create_pooled <- function(mice_df){
  
  w_selected = c("w_personid","w_htn","w_bmi","w_ge65","w_education_2", "w_education_3", "w_laborforce", "w_smoke", "w_heavydrinker", "w_moderate_pa", "w_vigorous_pa")
  h_selected = c("h_personid","h_htn","h_bmi", "h_ge65", "h_education_2", "h_education_3", "h_laborforce", "h_smoke", "h_heavydrinker", "h_moderate_pa", "h_vigorous_pa")
  hh_selected = c("psu", # Level 3
                  "coupleid", # Level 2
                  "hh_low", "hh_medium", "hh_high", "hh_highest", "hh_size",  "residence", "hh_lengthmar",
                  "in_religion","in_caste",
...
pooled_1 <- lasi_create_pooled(df_1)
...
bind_rows(
  ...
  write_csv(.,"lasi/g2al04_poisson regression for gendered differences.csv")
