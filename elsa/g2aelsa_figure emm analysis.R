htn <- read_csv("elsa/g2ae03_contrasts for poisson regression with multiple imputation.csv") %>% 
  mutate(outcome = "Hypertension")

htn_main <- read_csv("elsa/g2ae03_poisson regression with multiple imputation.csv") %>% 
...
tab_stratum %>% 
  ...
  write_csv(.,"elsa/table_emm analysis results.csv")
...
ggsave(figA,filename = paste0(path_g2a_family_folder,"/figures/g2aelsa_figure emm.png"),width = 10,height=6)
